...
int32_t main(int32_t argc, char **argv) {
    ...
    const int32_t plaintext_modulus = 2;
    const double alpha = 1e-6;
    const int32_t l = 3;
    const int32_t Bgbits = 10;
//TODO: parallelization
    static uniform_int_distribution<int32_t> unift(0, plaintext_modulus - 1);

    // PARAMETERS
    TLweParams *rlwe_params = new_TLweParams(N, k, plaintext_modulus, alpha_min_gsw, alpha_max_gsw); //les deux alpha mis un peu au hasard
    ...
    //the a plaintext_modulus message space.
    double expected_error_proba = 1. - erf(1. / (sqrt(2.) * 2. * plaintext_modulus * alpha));
    ...
    for (int32_t i = 0; i < N; ++i) {
        ...
        muB->coefsT[i] = modSwitchToTorus32(temp, plaintext_modulus);
    ...
    tLweSymDecrypt(dechifB, cipherB, rlwe_key, plaintext_modulus); // DECRYPTION
    for (int32_t i = 0; i < N; i++) {
        int32_t expected = modSwitchFromTorus32(muB->coefsT[i], plaintext_modulus);
        int32_t actual = modSwitchFromTorus32(dechifB->coefsT[i], plaintext_modulus);
        if (expected != actual)
    ...
    for (int32_t p = 0; p <= k; ++p) {
        for (int32_t i = 0; i < N; ++i) {
            ...
            int32_t expected2 = modSwitchFromTorus32(expected, plaintext_modulus);
            int32_t actual2 = modSwitchFromTorus32(actual, plaintext_modulus);
    ...
    tLweSymDecrypt(dechifAB, cipherAB, rlwe_key, plaintext_modulus); // DECRYPTION
    cout << "Test LweSymDecrypt after product 3.5 H*muB:" << endl;
    cout << " variance: " << cipherAB->current_variance << endl;
    for (int32_t i = 0; i < N; i++) {
        int32_t expected = modSwitchFromTorus32(muB->coefsT[i], plaintext_modulus);
        int32_t actual = modSwitchFromTorus32(dechifAB->coefsT[i], plaintext_modulus);
        if (expected != actual)
            printf("tlwe decryption error %d: %d != %d\n", i, actual, expected);
    }
    cout << "----------------------" << endl;
    ...
    tGswSymDecrypt(dechifA, cipherA, rgsw_key, plaintext_modulus);
    ...
    cout << " variance: " << cipherAB->current_variance << endl;
    for (int32_t i = 0; i < N; i++) {
        int32_t expected = modSwitchFromTorus32(muAB->coefsT[i], plaintext_modulus);
        int32_t actual = modSwitchFromTorus32(dechifAB->coefsT[i], plaintext_modulus);
        if (expected != actual)
            printf("tlwe decryption error %d: %d != %d\n", i, actual, expected);
    }
