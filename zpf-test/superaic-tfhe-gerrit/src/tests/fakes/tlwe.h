...
#define USE_FAKE_tLwePhase \
    ...
    inline void fake_tLweApproxPhase(TorusPolynomial *message, const TorusPolynomial *phase, int32_t plaintext_modulus, int32_t N) {
        torusPolynomialCopy(message, phase);
    }

#define USE_FAKE_tLweApproxPhase \
    inline void tLweApproxPhase(TorusPolynomial* message, const TorusPolynomial* phase, int32_t plaintext_modulus, int32_t N) { \
        fake_tLweApproxPhase(message, phase, plaintext_modulus, N); \
    }


    // Fake decryption of a Torus polynomial message
    inline void fake_tLweSymDecrypt(TorusPolynomial *result, const TLweSample *sample, const TLweKey *key, int32_t plaintext_modulus) {
        int32_t N = key->params->N;
        const FakeTLwe *fsamp = fake(sample);

        fake_tLweApproxPhase(result, fsamp->message, plaintext_modulus, N);
    }

#define USE_FAKE_tLweSymDecrypt \
    inline void tLweSymDecrypt(TorusPolynomial* result, const TLweSample* sample, const TLweKey* key, int32_t plaintext_modulus) { \
        fake_tLweSymDecrypt(result, sample, key, plaintext_modulus); \
    }


    // Fake decryption of a Torus message
    inline Torus32 fake_tLweSymDecryptT(const TLweSample *sample, const TLweKey *key, int32_t plaintext_modulus) {
        Torus32 message;
        const FakeTLwe *fsamp = fake(sample);
...
#define USE_FAKE_tLweSymDecryptT \
    inline Torus32 tLweSymDecryptT(const TLweSample* sample, const TLweKey* key, int32_t plaintext_modulus) { \
        return fake_tLweSymDecryptT(sample, key, plaintext_modulus); \
    }
