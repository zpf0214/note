...
#define USE_FAKE_tGswSymEncryptInt \
    ...
    inline void
    fake_tGswSymDecrypt(IntPolynomial *result, const TGswSample *sample, const TGswKey *key, const int32_t plaintext_modulus) {
        intPolynomialCopy(result, fake(sample)->message);
    }

#define USE_FAKE_tGswSymDecrypt \
    inline void tGswSymDecrypt(IntPolynomial* result, const TGswSample* sample, const TGswKey* key, const int32_t plaintext_modulus) { \
    fake_tGswSymDecrypt(result, sample, key, plaintext_modulus); \
    }
