...
namespace {
     ...
     */
    const TGswParams *params512_1 = new_TGswParams(4, 8, new_TLweParams(512, 1, 2, 0., 1.));
    const TGswParams *params512_2 = new_TGswParams(3, 10, new_TLweParams(512, 2, 2, 0., 1.));
    const TGswParams *params1024_1 = new_TGswParams(3, 10, new_TLweParams(1024, 1, 2, 0., 1.));
    const TGswParams *params1024_2 = new_TGswParams(4, 8, new_TLweParams(1024, 2, 2, 0., 1.));
    const TGswParams *params2048_1 = new_TGswParams(4, 8, new_TLweParams(2048, 1, 2, 0., 1.));
    const TGswParams *params2048_2 = new_TGswParams(3, 10, new_TLweParams(2048, 2, 2, 0., 1.));
...
#include "../libtfhe-superaic-core/tgsw-functions.cpp"
    };
...
#include "../libtfhe-superaic-core/tgsw-functions.cpp"
    ...
    //EXPORT void tGswSymDecrypt(IntPolynomial* result, const TGswSample* sample, const TGswKey* key, const int32_t plaintext_modulus);
    ...
    TEST_F(TGswTest, tGswClear) {
        for (const TGswParams *param: all_params) {
            ...
            for (int32_t i = 0; i < kpl; i++) {
                ...
                double norm = torusPolynomialNormInftyDist(si->message, zeroPol);
                printf("norm is %f \n",norm);
                ASSERT_EQ(norm, 0);
            }
