...
EXPORT void lweKeyGen(LweKey* result) {
  ...
  for (int32_t i=0; i<n; i++) 
    result->key[i]=_use_fix_random ? 0 : distribution(generator);
}
 ...
 * The constant plaintext_modulus indicates the message space and is used to approximate the phase
 */
...
void init_LweSample(LweSample* obj, const LweParams* params) {
    new(obj) LweSample(params);
}

void init_LweSample(LweSample* obj, const std::shared_ptr<LweParams> params) {
    new(obj) LweSample(params);
}
