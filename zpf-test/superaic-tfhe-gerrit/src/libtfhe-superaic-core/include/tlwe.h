...
struct TLweParams {
    ...
    TLweParams(int32_t N, int32_t k, int32_t plaintext_modulus, double alpha_min, double alpha_max);
...
struct TLweKey {
    // params 和传入的params指向同一个对象，因此不能在释放TLweKey对象之前释放构造对象的时候所引用的
    // TLweParams 对象，会导致程序崩溃。
    // TODO : 改为共享指针 ， 要严格保证params 在 params_shared之后声明
    std::shared_ptr<TLweParams> params_shared;
    ...
    TLweKey(std::shared_ptr<TLweParams> params);
...
struct TLweSample {
    ...
    TLweSample(std::shared_ptr<TLweParams> params);
 ...
 */
void init_TLweKey(TLweKey *ptr, const TLweParams *params);
 ...
 * @param params the LWE parameters to use
 */
void init_TLweSample(TLweSample *ptr, const TLweParams *params);
