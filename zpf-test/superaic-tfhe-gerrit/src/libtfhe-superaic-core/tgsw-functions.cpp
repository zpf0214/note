...
EXPORT void tGswSymDecrypt(IntPolynomial *result, const TGswSample *sample, const TGswKey *key, const int32_t plaintext_modulus) {
    ...
    const Torus32 indic = modSwitchToTorus32(1, plaintext_modulus);
    ...
    for (int32_t i = 0; i < N; i++)
        result->coefs[i] = modSwitchFromTorus32(testvec->coefsT[i], plaintext_modulus);
...
EXPORT int32_t tGswSymDecryptInt(const TGswSample* sample, const TGswKey* key){
    ...
    int32_t result = modSwitchFromTorus32(phase->coefsT[0], plaintext_modulus);
...
// tgsw_test 里用了一些奇怪的操作，只能定义在这里，否则测试不过
...
EXPORT void init_TGswSample_array(int32_t nbelts, TGswSample* obj, const TGswParams* params) { 
    for (int32_t ii=0; ii<nbelts; ++ii) init_TGswSample(obj+ii,params); 
} 

/* destroy array */ 
EXPORT void destroy_TGswSample_array(int32_t nbelts, TGswSample* obj) { 
    for (int32_t ii=0; ii<nbelts; ++ii) destroy_TGswSample(obj+ii); 
} 
...
EXPORT TGswSample* new_TGswSample_array(int32_t nbelts, const TGswParams* params) { 
    TGswSample* reps = alloc_TGswSample_array(nbelts); 
    init_TGswSample_array(nbelts, reps, params); 
    return reps; 
} 

/* destroy+free */ 
EXPORT void delete_TGswSample(TGswSample* obj) { 
    destroy_TGswSample(obj); 
    free_TGswSample(obj); 
} 
