...
//void (*torusPolynomialAddMulR)(TorusPolynomial* result, const IntPolynomial* poly1, const TorusPolynomial* poly2) = torusPolynomialAddMulRNaive;
function<void(TorusPolynomial* , const IntPolynomial* , const TorusPolynomial* poly2)> torusPolynomialAddMulR = torusPolynomialAddMulRNaive;
mutex torusPolynomialAddMulR_mtx;
...
void IntPolynomial::print(void) const {
	printf("IntPolynomial:\n");
	printf("N:%d\n",N);
	int data_perline = 100;
	int line_count = data_perline;
	for( int i = 0; i < N;i++) {
		printf("%d ",coefs[i]);
		line_count --;
		if(line_count == 0) {
			printf("\n");
			line_count = data_perline;
		}
	}
	printf("\n");
}

void TorusPolynomial::print(bool bRaw) const {
	if (!bRaw){
		printf("TorusPolynomial:\n");
		printf("N:%d\n",N);
	}
	int data_perline = 100;
	int line_count = data_perline;
	for( int i = 0; i < N;i++) {
		printf("0x%08x ",coefsT[i]);
		line_count --;
		if(line_count == 0) {
			printf("\n");
			line_count = data_perline;
 ...
 */
EXPORT void torusPolynomialAddMulRNaive(TorusPolynomial* result, const IntPolynomial* poly1, const TorusPolynomial* poly2) {
    const int32_t N = poly1->N;
    assert(result!=poly2);
    assert(poly2->N==N && result->N==N);
 ...
 * result as the karatsuba or fft version, but should be slower) 
 */
EXPORT void torusPolynomialSubMulRNaive(TorusPolynomial* result, const IntPolynomial* poly1, const TorusPolynomial* poly2) {
    const int32_t N = poly1->N;
    assert(result!=poly2);
    assert(poly2->N==N && result->N==N);
	TorusPolynomial temp(N);
